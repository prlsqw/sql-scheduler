...
void initialize(Dataframe *df, const char *file_path) {
	...
	void initialize(Dataframe * df, const char *file_path) {
		...
		df->row_width = ((df->num_cols * df->cell_length) // for each cell
	...
	void execute(Dataframe * df, ExecutionState * state, time_t timeout) {
		...
		void execute_average(Dataframe * df, ExecutionState * state,
							 time_t timeout) {
			void execute_average(Dataframe * df, ExecutionState * state,
								 time_t timeout) {
				...
				printf("AVERAGE(%d): %f\n", column_index,
					   state->tally / (df->num_rows - 1));
				printf("AVERAGE(%d): %f\n", column_index,
					   state->tally / (df->num_rows - 1));
			}

			void execute_median(Dataframe * df, ExecutionState * state,
								time_t timeout) {
				int column_index = state->query->column_index;
				time_t start_time = now();
				...
				if (state->status == CREATED) {
					...
					state->values =
						malloc(state->values_capacity * sizeof(double));
				...
				while ((state->processed_rows < df->num_rows - 1) &&
					...
					insert_sorted(&state->values, &count,
								  &state->values_capacity, value);
				...
				if (n % 2 == 1) {
					...
					median =
						(state->values[n / 2 - 1] + state->values[n / 2]) / 2.0;
				}
			...
			void execute_increment(Dataframe * df, ExecutionState * state,
								   time_t timeout) {
				int column_index = state->query->column_index;
			...
			void execute_write(Dataframe * df, ExecutionState * state,
							   time_t timeout) {
				int column_index = state->query->column_index;
				double value = state->query->arg1;
				time_t start_time = now();
			...
			void execute_write_at(Dataframe * df, ExecutionState * state,
								  time_t timeout) {
				int column_index = state->query->column_index;
				int row_index = (int)state->query->arg1;
				double value = state->query->arg2;
				// local flag to keep track of printing WRITE_AT only when the
				// user calls it
			...
			void execute_count(Dataframe * df, ExecutionState * state,
							   time_t timeout) {
				...
				if (state->status == CREATED) {
					...
					void execute_count(Dataframe * df, ExecutionState * state,
						...
						if (state->status == CREATED) {
							...
							if (column_index < 0 ||
								column_index >= df->num_cols) {
								perror(
									"COUNT Error: Column index out of range");
								exit(1);
							}
						}
					...
					printf("COUNT(%d, %c, %f): %d\n", column_index,
						   (char)comparison_operator, value, (int)state->tally);
					printf("COUNT(%d, %c, %f): %d\n", column_index,
						   (char)comparison_operator, value, (int)state->tally);
				}
