...
void execute_median(Dataframe *df, ExecutionState *state, time_t timeout) {
	...
	// in each row, find the value at column_index and insert into sorted array
	while ((state->processed_rows < df->num_rows - 1) &&
		   (now() - start_time < timeout)) {
		read_value_at_column(df->file, column_index, buffer);
		...
		// (read_value_at_column for the last column already consumes the
		// newline)
...
void execute_count(Dataframe *df, ExecutionState *state, time_t timeout) {
	...
	printf("COUNT(%d, %c, %f): %d\n", column_index, (char)comparison_operator,
		   value, (int)state->tally);
}
