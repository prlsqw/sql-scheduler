...
def run_benchmark(data_file, queries_file):
    ...
    for algo in algos:
        log_file = f"log_{algo}.csv"
        # Cleanup previous logs
        if os.path.exists(log_file):
            os.remove(log_file)
        
        # Clean weights for fair comparison (WRR builds history)
        ...
        # Construct command: cat queries | python3 feeder.py 0.05 | ./orchestrator data algo log
        # 0.05s delay to create some concurrency pressure without taking forever
        cmd = f"cat {queries_file} | python3 feeder.py 0.05 | ./orchestrator {data_file} {algo} {log_file}"
        
        try:
            # We use shell=True to handle the pipes easily
    ...
    # Determine winner based on Turnaround Time (common metric)
    best_algo = None
